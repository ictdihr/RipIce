...

{
    class PrintDGV
    {
        ...
        private static ArrayList RowHeights = new ArrayList(); // Height of Rows // 25/11/2020

        private static ArrayList ColumnTypes = new ArrayList();  // DataType of Columns
        private static ArrayList ColumnStylesR = new ArrayList();  // DataStyle of Columns // 25/11/2020
        ...
        private static string Orientamento = "";  // Orientamento V o L  20/11/2020
        private static bool StampaLogo = false;  // Se True stampa il logo in alto a Sinistra
        private static Image logostampa;         // Logo da Stampare
        ...
        public static void Print_DataGridView(DataGridView dgv1,string titoloStampa,PictureBox logoSt)
        {
	        ...
	        {	
                ...
                PrintOptions dlg = new PrintOptions(AvailableColumns,titoloStampa, logoSt);
                if (dlg.ShowDialog() != DialogResult.OK) return;

                //20/11/2020
                if (dlg.radioButtonV.Checked)
                {
                    Orientamento = "V";
                }
                else
                ...
                StampaLogo = dlg.checkStampaLogo.Checked;

                logostampa = logoSt.Image;
                ...
                if (PageNo == 1) 
                {
                    foreach (DataGridViewColumn GridCol in dgv.Columns)
                    {
                        ...
                        HeaderHeight = (int)(e.Graphics.MeasureString(GridCol.HeaderText,
                                    GridCol.InheritedStyle.Font, tmpWidth).Height) + 15; // 11
                        
                        // Save width & height of headres and ColumnType
                        ColumnLefts.Add(tmpLeft);
                        ColumnWidths.Add(tmpWidth);
                        ColumnTypes.Add(GridCol.GetType());
                        ColumnStylesR.Add(GridCol.DefaultCellStyle); //25/11/2020
                        tmpLeft += tmpWidth;
                    }
                }
                ...
                while (RowPos <= dgv.Rows.Count - 1)
                {
                    ...
                    else
                    {
                        if (NewPage)
                        {
                            ...
                            if (StampaLogo)
                            {
                                //e.Graphics.DrawImage(logostampa, 1, 1);
                                Rectangle CelSize = new Rectangle(e.MarginBounds.Left / 2, e.MarginBounds.Top +1, 100, 30);
                                Size ImgSize = ((Image)(logostampa)).Size;
                                e.Graphics.DrawImage((Image)logostampa,
                                        new Rectangle(e.MarginBounds.Left + (e.MarginBounds.Width/2) - (CelSize.Width/2) , e.MarginBounds.Top - (CelSize.Height ) -10, 100, 30));
                            }
                        ...
                        foreach (DataGridViewCell Cel in GridRow.Cells)
                        {
                            ...
                            {
                                if ( ((DataGridViewCellStyle) ColumnStylesR[i]).Alignment == DataGridViewContentAlignment.MiddleCenter)
                                {
